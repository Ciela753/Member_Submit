...
public class ReplyDao {
	...
	public void insert(Reply reply) {
		...
		try {
			PreparedStatement pstmt = conn.prepareStatement(
					"INSERT INTO USER26.TBL_REPLY(RNO, TITLE, CONTENT, ID, BNO) VALUES (SEQ_REPLY.NEXTVAL,?,?,?,?)");
			int idx=1;
	...
	public List<Reply> list(Long bno) {
		...
		try {
			String sql = "SELECT RNO, TITLE, CONTENT, TO_CHAR(REGDATE, 'YY/MM/DD') AS REGDATE, ID, BNO\r\n" +
					"FROM USER26.TBL_REPLY tr WHERE RNO > 0 AND BNO = ?";
	...
	public Reply select(Long rno) {
		...
		try {
			String sql = "SELECT\r\n"+
					...
					"FROM USER26.TBL_REPLY WHERE RNO = ?";
	...
	public void delete(Long rno) {
		...
		try {
			PreparedStatement pstmt = conn.prepareStatement(
						"DELETE USER26.TBL_REPLY WHERE RNO = ?");
			int idx = 1;
